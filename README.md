...
3주차
1. 마을에서는 마을BGM이 재생 (8.3)
2. 마을과 게임 씬에서 화면 UI로 캐릭터의 HP, 경험치, 스킬, 레벨, 코인 UI와 캐릭터 정보 버튼, 퀘스트 버튼, 스킬 버튼, 인벤토리 버튼, 옵션 버튼 배치 (5.2, 7.2)
3. 인벤토리 버튼을 누를 시 현재 캐릭터가 착용 중인 장비 정보와 소지 중인 아이템들을 볼 수 있는 UI생성 (9.5)
4. 인벤토리에는 아이템 정렬, 이동, 삭제 및 배낭 기능 구현 (9.6)
5. 검, 갑옷, 물약을 획득 시에는 인벤토리 안으로 들어감 (10.1)

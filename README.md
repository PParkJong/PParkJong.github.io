...
| No | 화면 |   | 용어 |설명|
|:----:|:----:|:----:|:----:|:----:|
| 1 | Title{시작화면} | | |Dungeon Breaker의 첫 화면이다. 타이틀, 시작하기 버튼, 종료하기 버튼이 있다.|
| 2 | Title{시작화면} | 화면 요소 | 타이틀 | Dungeon Breaker의 타이틀이다. |
| 3 | Title{시작화면} | 화면 요소 | ①시작하기 버튼 | 클릭 시 로딩 화면을 거쳐 캐릭터 선택 화면으로 넘어간다. |
| 4 | Title{시작화면} | 화면 요소 | ②종료하기 버튼 | 클릭 시 게임을 종료한다. |
|  |  |  |  |  |
| 5 | Loading{로딩화면} | | | 다음 화면으로 가기 전의 로딩 화면이다. 개발자의 말, 로딩바가 있다. |
| 6 | Loading{로딩화면} | 화면 요소 | 개발자의 말 | 개발자가 플레이어에게 하고싶은 말이 적혀있다. |
| 7 | Loading{로딩화면} | 화면 요소 | 로딩바 | 현재 로딩되고 있는 현황을 보여준다. |
|  |  |  |  |  |
| 8 | Charactger Select{캐릭터 선택 화면} | | | 캐릭터 선택 화면이다. 캐릭터를 선택하면 캐릭터 정보, 시작하기 버튼이 있다. |
| 9 | Charactger Select{캐릭터 선택 화면} | 화면 요소 | 캐릭터 | 플레이어가 조종할 캐릭터다. |
| 10 | Charactger Select{캐릭터 선택 화면} | 화면 요소 | ③캐릭터 정보바 | 캐릭터의 레벨과 능력치가 표시되어 있다. |
| 11 | Charactger Select{캐릭터 선택 화면} | 화면 요소 | ①시작하기 버튼 | 클릭 시 로딩 화면을 거쳐 게임 화면으로 넘어간다. |
|  |  |  |  |  |
| 12 | Play{게임화면} | | | 플레이하는 게임 화면이다. 플레이어 캐릭터와 다양한 정보 UI와 버튼이 있다. |
| 13 | Play{게임화면} | 화면 요소 | 캐릭터 | 플레이어가 조종하는 캐릭터다. |
| 14 | Play{게임화면} | 화면 요소 | 레벨 | 플레이어의 레벨을 표시한다. |
| 15 | Play{게임화면} | 화면 요소 | 경험치 | 플레이어의 경험치를 표시한다. |
| 16 | Play{게임화면} | 화면 요소 | Hp | 플레이어의 Hp를 표시한다. |
| 17 | Play{게임화면} | 화면 요소 | 스킬 | 플레이어의 스킬을 표시한다. |
| 18 | Play{게임화면} | 화면 요소 | 코인 | 플레이어의 소지 중인 코인을 표시한다. |
| 19 | Play{게임화면} | 화면 요소 | 캐릭터 정보 버튼 | 클릭 시 ③캐릭터 정보가 표시된 UI가 나온다. |
| 20 | Play{게임화면} | 화면 요소 | 퀘스트 버튼 | 클릭 시 진행 중인 퀘스트 정보가 나온다. |
| 21 | Play{게임화면} | 화면 요소 | 스킬 버튼 | 클릭 시 플레이어의 스킬 정보가 나온다. |
| 22 | Play{게임화면} | 화면 요소 | 인벤토리 버튼 | 클릭 시 캐릭터가 착용 중인 장비, 소지 중인 아이템이 나온다. |
| 23 | Play{게임화면} | 화면 요소 | 옵션 버튼 | 클릭 시 소리 옵션과 ②종료하기 버튼이 나온다. |
|  |  |  |  |  |
| 24 | Play{게임화면}{던전} | 화면 요소 | 미션 | 다음 맵으로 넘어가기 위한 조건을 표시한다. |
|  |  |  |  |  |
| 25 | Play{게임화면} | 스크립트 요소 | 키 | 총 12가지의 키(방향키, 스페이스 바, 공격키, 스킬키)가 있다. |
| 26 | Play{게임화면} | 스크립트 요소 | UP | 위 방향키 up arrow이다. |
| 27 | Play{게임화면} | 스크립트 요소 | DOWN | 아래 방향키 down arrow이다. |
| 28 | Play{게임화면} | 스크립트 요소 | LEFT | 왼쪽 방향키 left arrow이다. |
| 29 | Play{게임화면} | 스크립트 요소 | RIGHT | 오른쪽 방향키 right arrow이다. |
| 30 | Play{게임화면} | 스크립트 요소 | SPACE | 스페이스 바 space이다. |
| 31 | Play{게임화면} | 스크립트 요소 | 공격키 | 공격키 x이다. |
| 32 | Play{게임화면} | 스크립트 요소 | 스킬키 | 스킬키 a, s, d, q, w, e이다. |
| 33 | Play{게임화면} | 스크립트 요소 | p_Hp | 플레이어의 Hp다. 값은 100이다. |
| 34 | Play{게임화면} | 스크립트 요소 | p_Lv | 플레이어의 레벨이다. |
| 35 | Play{게임화면} | 스크립트 요소 | p_Exp | 플레이어의 경험치다. 값은 100이다. |
| 36 | Play{게임화면} | 스크립트 요소 | p_Atk | 플레이어의 공격력이다. |
| 37 | Play{게임화면} | 스크립트 요소 | p_Speed | 플레이어의 이동속도이다. 회피기 사용시 일시적으로 증가한다. |
| 38 | Play{게임화면} | 스크립트 요소 | s_Cool | 스킬의 쿨타임이다. 스킬마다 쿨타임이 다르다. |
| 39 | Play{게임화면} | 스크립트 요소 | c_Amount | 플레이어가 소지 중인 코인갯수다. |
| 40 | Play{게임화면} | 스크립트 요소 | mission_Info | 현재 맵에 있는 몬스터의 수이다. 0/20와 같이 표시한다. |
| 41 | Play{게임화면} | 스크립트 요소 | d_Time | 던전에서의 남은 시간이다. 00:00와 같이 표시한다. |
| 42 | Play{게임화면} | 스크립트 요소 | audio | 게임 장소에 맞는 bgm이다. |

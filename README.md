...
## 1. 요구사항
- 씬(타이틀, 로딩, 캐릭터 선택, 마을, 던전) 총 5종류
- 타이틀 씬에는 타이틀, 시작하기 버튼, 종료하기 버튼 배치
- 타이틀BGM 재생
- 시작하기를 누를 시 캐릭터 선택 씬으로 변경
- BGM이 재생을 중단 후 밤의 숲의 효과음 재생
- 씬 변경시에는 로딩 화면을 거쳐 씬 변경
- 종료하기를 누를 시 게임이 종료
- 캐릭터 선택 씬에서 캐릭터를 선택할 시 캐릭터에 대한 정보 UI와 시작하기 버튼이 배치
- 시작하기를 누를 시 마을로 씬 변경
- 마을에서는 마을BGM이 재생
- 마을에는 성과 다양한 건물들 배치
- 마을과 게임 씬에서 화면 UI로 캐릭터의 HP, 경험치, 스킬, 레벨, 코인 UI와 캐릭터 정보 버튼, 퀘스트 버튼, 스킬 버튼, 인벤토리 버튼, 옵션 버튼 배치
- 플레이어 캐릭터는 화면 중앙에 있으며 카메라는 고정된 위치에서 플레이어를 비춤
- 캐릭터는 방향키로 움직이며 같은 방향키를 연속으로 입력 시 달림
- SPACE키로 짧은 거리를 이동하는 대쉬 구현
- x키로 기본 공격 구현
- a, s, d, q, w, e키로 스킬 구현
- 버튼을 눌러 나오는 UI는 UI의 우측상단 x버튼으로 닫기 가능
- 캐릭터 정보 버튼을 누를 시 캐릭터가 현재 착용하고 있는 장비 및 캐릭터의 능력치가 나오는 UI생성
- 퀘스트 버튼을 누를 시 현재 진행 중인 퀘스트 정보가 나오는 UI생성
- 스킬 버튼을 누를 시 캐릭터의 스킬 정보를 볼 수 있는 UI생성
- 인벤토리 버튼을 누를 시 현재 캐릭터가 착용 중인 장비 정보와 소지 중인 아이템들을 볼 수 있는 UI생성
- 인벤토리에는 아이템 정렬, 이동, 삭제 및 배낭, 정렬 기능 구현
- 옵션 버튼을 누를 시 소리 조절 등의 기능과 게임종료 버튼을 배치
- 마을에는 던전으로 이동할 수 있는 포탈과 상점 그리고 NPC 배치
- 상점은 아이템 구매와 판매 가능
- 특정 NPC의 경우 퀘스트를 제공
- 퀘스트는 수락할 수도 거절할 수도 있음
- 퀘스트는 클리어할 시 정해진 보상을 얻을 수 있음
- 던전으로 이동할 수 있는 포탈에게 다가가면 입장하고 싶은 던전을 선택할 수 있는 UI생성
- 원하는 던전을 선택 시 던전의 정보와 입장하기 버튼이 있는 UI가 추가 생성
- 입장하기 버튼을 누를 시 던전 씬으로 변경
- 던전 입장시 마을BGM에서 던전BGM으로 변경
- 던전은 숲을 배경
- 던전에 입장 시 오른쪽에 미션이 나오며 미션을 해결해야 다음 맵으로 가는 포탈이 열림
- 몬스터는 배회를 하고 있으며 플레이어가 감지 범위 내에 들어올 시 추격
- 플레이어는 공격과 스킬을 통해 몬스터들을 공격하여 처치
- 몬스터는 피격시 몸이 잠시 붉게 변한 후 원래대로 돌아옴
- 플레이어 HP가 없는 경우 물약을 통해 회복
- 보스방에 입장 시 BGM이 던전BGM에서 보스BGM으로 변경
- 보스방에 입장 시 보스를 잠시 클로즈업 후 다시 원래 화면으로 변경
- 보스 몬스터는 배회 없이 플레이어를 항상 감지하고 추격
- 보스 몬스터가 기술을 사용할 시에는 미리 공격의 경로 및 범위가 바닥에 표시
- 보스 몬스터를 쓰러뜨리면 상자가 생성
- 상자를 열면 랜덤으로 보상이 생성
- 보상에는 검, 갑옷, 물약, 코인, 경험치가 있음
- 검, 갑옷, 물약을 획득 시에는 인벤토리 안으로 들어감
- 코인을 획득 시에는 코인 UI의 코인 숫자가 증가
- 경험치를 획득 시에는 플레이어의 현재 경험치량이 변함
- 경험치가 100%가 넘을 시 플레이어의 레벨이 1 증가
- 플레이어가 특정 레벨을 도달할 때 마다 새로운 스킬을 흭득
<br>
